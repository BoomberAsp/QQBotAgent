...
# The §4 taxonomy the model may answer with (semantic hints included).
_TRIGGER_CLASSES = (
    "battle_start", "on_skill_use", "on_ally_attack", "on_ally_skill",
    "on_ally_aoe", "on_ally_nonattack", "on_ally_turn_end", "on_ally_crit",
    "on_self_crit", "on_hit", "on_follow_up", "on_evade", "on_enemy_skill",
    "on_enemy_turn_end", "on_kill", "on_death", "conditional",
)
...
- on_evade 自身闪避时
- on_enemy_skill 敌人施展非攻击技能时
- on_enemy_turn_end 敌方成员回合结束时
- on_kill 消灭敌人时 / on_death 任一成员死亡时
- conditional 纯条件（HP/状态/战意/冷却，无明确事件触发）
